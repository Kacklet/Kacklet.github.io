...
public abstract class MixinItemRenderer {
    ...
    public void renderItemInFirstPerson(final float partialTicks) {
        ...
        final boolean shouldSpoofBlocking = (Magma.killAura.isToggled() && KillAura.target != null && !Magma.killAura.blockMode.getSelected().equals("None"));
        if (this.itemToRender != null) {
            if (this.itemToRender.getItem() instanceof ItemMap) {
                ...
                switch (enumaction) {
                    ...
                    case BLOCK: {
                        if (Magma.animations.isToggled()) {
                            final String selected = Magma.animations.mode.getSelected();
                            switch (selected) {
                                case "1.7": {
                                ...
                                case "spin":
                                case "vertical spin": {
                                    this.transformFirstPersonItem(f, Magma.animations.showSwing.isEnabled() ? f2 : 0.0f);
                                    this.doBlockTransformations();
                                    break;
                                }
                                case "long hit": {
                                ...
                                case "custom": {
                                    this.transformFirstPersonItem(Magma.animator.blockProgress.isEnabled() ? f : 0.0f, Magma.animator.swingProgress.isEnabled() ? f2 : 0.0f);
                                    this.doBlockTransformations();
                                    break;
                                }
                                case "helicopter": {
    ...
    private void transformFirstPersonItem(final float equipProgress, final float swingProgress) {
        if (Magma.animations.isToggled()) {
            final float size = (float) Magma.animations.size.getValue();
            final float x = (float) Magma.animations.x.getValue();
            final float y = (float) Magma.animations.y.getValue();
            final float z = (float) Magma.animations.z.getValue();
    ...
    private void doBlockTransformations() {
        ...
        final String selected = Magma.animations.mode.getSelected();
        if(Magma.animations != null && Magma.animations.isToggled()) {
            switch (selected) {
                case "custom": {
                    angle1 = (float) Magma.animator.angle1.getValue();
                    angle2 = (float) Magma.animator.angle2.getValue();
                    angle3 = (float) Magma.animator.angle3.getValue();
                    translateX = (float) Magma.animator.translateX.getValue();
                    translateY = (float) Magma.animator.translateY.getValue();
                    translateZ = (float) Magma.animator.translateZ.getValue();
                    rotation1x = (float) Magma.animator.rotation1x.getValue();
                    rotation1y = (float) Magma.animator.rotation1y.getValue();
                    rotation1z = (float) Magma.animator.rotation1z.getValue();
                    rotation2x = (float) Magma.animator.rotation2x.getValue();
                    rotation2y = (float) Magma.animator.rotation2y.getValue();
                    rotation2z = (float) Magma.animator.rotation2z.getValue();
                    break;
                }
                case "vertical spin": {
