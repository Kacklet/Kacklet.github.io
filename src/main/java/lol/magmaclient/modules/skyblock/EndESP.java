...
import lol.magmaclient.utils.objects.BlockPosition;
...
public class EndESP extends Module
{
    ...
    public void onParticle(PacketReceivedEvent event) {
        ...
        if (event.packet instanceof S2APacketParticles) {
            ...
            if (packet.getParticleType().equals(EnumParticleTypes.PORTAL)) {
                ...
                IBlockState blockState = Magma.mc.theWorld.getBlockState(blockPosition.toBlockPos());
    ...
    public void onRenderWorld(RenderWorldLastEvent event) {
        if (!this.isToggled() || Magma.mc.thePlayer == null) return;
        ...
        for (BlockPosition blockPosition : enderNodes.keySet()) {
            RenderUtils.renderEspBox(blockPosition.toBlockPos(), event.partialTicks, Magma.themeManager.getSecondaryColor().getRGB());
            if(this.nodesTracer.isEnabled()) {
                RenderUtils.tracerLine(blockPosition.x + 0.5, blockPosition.y + 0.5, blockPosition.z + 0.5, Magma.themeManager.getSecondaryColor());
            }
        }
    }
