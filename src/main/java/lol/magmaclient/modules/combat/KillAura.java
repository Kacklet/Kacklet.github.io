...
public class KillAura extends Module
{
    ...
    public void onTick(final TickEvent.ClientTickEvent event) {
        if (Magma.mc.thePlayer == null || Magma.mc.theWorld == null || !this.middleClick.isEnabled()) {
            return;
        }
        if (Mouse.isButtonDown(2) && Magma.mc.currentScreen == null) {
            if (Magma.mc.pointedEntity != null && !this.wasDown && !(Magma.mc.pointedEntity instanceof EntityArmorStand) && Magma.mc.pointedEntity instanceof EntityLivingBase) {
                final String name = ChatFormatting.stripFormatting(Magma.mc.pointedEntity.getName());
                if (!KillAura.names.contains(name)) {
                    KillAura.names.add(name);
                    Magma.notificationManager.showNotification("Added " + ChatFormatting.AQUA + name + ChatFormatting.RESET + " to name sorting", 2000, Notification.NotificationType.INFO);
                }
                else {
                    KillAura.names.remove(name);
                    Magma.notificationManager.showNotification("Removed " + ChatFormatting.AQUA + name + ChatFormatting.RESET + " from name sorting", 2000, Notification.NotificationType.INFO);
                }
            }
            this.wasDown = true;
        }
    ...
    public void onEnable() {
        if(syncDisplay.isEnabled()) {
            Magma.targetDisplay.setToggled(true);
        }
        this.attacks = 0;
    }
    ...
    public void onDisable() {
        if(syncDisplay.isEnabled()) {
            Magma.targetDisplay.setToggled(false);
        }
    ...
    public void onMovePre(final MotionUpdateEvent.Pre event) {
        ...
        if (!this.isToggled() || (this.attackOnly.isEnabled() && !Magma.mc.gameSettings.keyBindAttack.isKeyDown())) {
            return;
        }
        if (KillAura.target != null) {
            ...
            if (this.shovelSwap.isEnabled() && KillAura.target instanceof EntityPlayer && this.hasDiamondArmor((EntityPlayer)KillAura.target)) {
                this.lastSlot = Magma.mc.thePlayer.inventory.currentItem;
                for (int i = 0; i < 9; ++i) {
                    if (Magma.mc.thePlayer.inventory.getStackInSlot(i) != null && Magma.mc.thePlayer.inventory.getStackInSlot(i).getItem() instanceof ItemSpade) {
    ...
    public void onMovePost(final MotionUpdateEvent.Post event) {
        ...
        if (this.attackOnly.isEnabled() && !Magma.mc.gameSettings.keyBindAttack.isKeyDown()) {
            this.attacks = 0;
            return;
        }
        if (KillAura.target != null && Magma.mc.thePlayer.getDistanceToEntity((Entity)KillAura.target) < Math.max(this.rotationRange.getValue(), this.range.getValue()) && this.attacks > 0) {
            ...
            while (this.attacks > 0) {
                ...
                if (Magma.mc.thePlayer.getDistanceToEntity((Entity)KillAura.target) < this.range.getValue() && (RotationUtils.getRotationDifference(RotationUtils.getRotations(KillAura.target), RotationUtils.getLastReportedRotation()) < 15.0 || this.rotationMode.is("None"))) {
                    Magma.mc.playerController.attackEntity((EntityPlayer) Magma.mc.thePlayer, (Entity)KillAura.target);
            ...
            if (Magma.mc.thePlayer.getHeldItem() != null && Magma.mc.thePlayer.getHeldItem().getItem() instanceof ItemSword) {
                final String selected2 = this.blockMode.getSelected();
                switch (selected2) {
                    case "Vanilla": {
                        if (!this.isBlocking) {
    ...
    public void onRender(final RenderWorldLastEvent event) {
        if (this.isToggled() && KillAura.target != null && this.lastAttack.hasTimePassed(1000 / this.nextCps) && Magma.mc.thePlayer.getDistanceToEntity((Entity)KillAura.target) < (this.rotationSwing.isEnabled() ? this.getRotationRange() : this.range.getValue())) {
    ...
    private EntityLivingBase getTarget() {
        if ((Magma.mc.currentScreen instanceof GuiContainer && this.toggleInGui.isEnabled()) || Magma.mc.theWorld == null) {
            return null;
        }
        final List<Entity> validTargets = (List<Entity>) Magma.mc.theWorld.getLoadedEntityList().stream().filter(entity -> entity instanceof EntityLivingBase).filter(entity -> this.isValid((EntityLivingBase) entity)).sorted(Comparator.comparingDouble(e -> e.getDistanceToEntity((Entity) Magma.mc.thePlayer))).collect(Collectors.toList());
    ...
    private boolean isValid(final EntityLivingBase entity) {
        if (entity == Magma.mc.thePlayer || !AntiBot.isValidEntity((Entity)entity) || (!this.invisibles.isEnabled() && entity.isInvisible()) || entity instanceof EntityArmorStand || (!Magma.mc.thePlayer.canEntityBeSeen((Entity)entity) && !this.walls.isEnabled()) || entity.getHealth() <= 0.0f || entity.getDistanceToEntity((Entity) Magma.mc.thePlayer) > ((KillAura.target != null && KillAura.target != entity) ? this.range.getValue() : Math.max(this.rotationRange.getValue(), this.range.getValue())) || RotationUtils.getRotationDifference(RotationUtils.getRotations(entity), RotationUtils.getPlayerRotation()) > this.fov.getValue()) {
            return false;
        }
    ...
    private void startBlocking() {
        PacketUtils.sendPacketNoEvent((Packet<?>)new C08PacketPlayerBlockPlacement(Magma.mc.thePlayer.getHeldItem()));
        this.isBlocking = true;
    }

    private void stopBlocking() {
        if (this.isBlocking) {
            Magma.mc.getNetHandler().getNetworkManager().sendPacket((Packet)new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, EnumFacing.DOWN));
            this.isBlocking = false;
        }
    }
