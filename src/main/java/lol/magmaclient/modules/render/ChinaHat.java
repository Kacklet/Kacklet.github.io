...
public class ChinaHat extends Module
{
    ...
    public void onRender(final RenderWorldLastEvent event) {
        if (this.isToggled() && (Magma.mc.gameSettings.thirdPersonView != 0 || this.firstPerson.isEnabled())) {
            this.drawChinaHat((EntityLivingBase) Magma.mc.thePlayer, event.partialTicks);
        }
    }
    ...
    private void drawChinaHat(final EntityLivingBase entity, final float partialTicks) {
        ...
        GL11.glTranslated(entity.lastTickPosX + (entity.posX - entity.lastTickPosX) * partialTicks - Magma.mc.getRenderManager().viewerPosX, entity.lastTickPosY + (entity.posY - entity.lastTickPosY) * partialTicks - Magma.mc.getRenderManager().viewerPosY + entity.height + (entity.isSneaking() ? (this.pos.getValue() - 0.23000000417232513) : this.pos.getValue()), entity.lastTickPosZ + (entity.posZ - entity.lastTickPosZ) * partialTicks - Magma.mc.getRenderManager().viewerPosZ);
        GL11.glRotatef((float)((entity.ticksExisted + partialTicks) * this.rotation.getValue()) - 90.0f, 0.0f, 1.0f, 0.0f);
        GL11.glRotatef(-(Magma.mc.thePlayer.prevRotationYawHead + (Magma.mc.thePlayer.rotationYawHead - Magma.mc.thePlayer.prevRotationYawHead) * partialTicks), 0.0f, 1.0f, 0.0f);
    ...
    public Color getColor(final double i, final double max, final boolean first) {
        ...
        return Magma.clickGui.getColor((int)c);
    }
}
