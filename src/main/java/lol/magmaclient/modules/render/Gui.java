...
public class Gui extends Module {
    ...
    public Gui() {
        super("Gui", Keyboard.KEY_RETURN, Category.RENDER);
        this.colorMode = new ModeSetting("Theme", "Katoz", new String[] { "Katoz", "Gradient", "Rainbow", "Vape", "Mint", "Devil"})
        {
            ...
            public void cycle(int key)
            {
                ...
                Magma.themeManager.setTheme(colorMode.getSelected());
            }
        };
    ...
    public float getHeight() {
        ...
        final List<Module> list = Magma.moduleManager.getModules().stream().filter(module ->  (module.isToggled() || module.toggledTime.getTimePassed() <= 250L)).sorted(Comparator.comparingDouble(module -> Fonts.getPrimary().getStringWidth(module.getName()))).collect(Collectors.toList());
        float y = 3.0f;
        ...
        if (event.type == RenderGameOverlayEvent.ElementType.CROSSHAIRS) {
            if (this.waterMark.isEnabled()) {
                if(Boolean.parseBoolean(Magma.licensed)) {
                    Fonts.getSecondary().drawSmoothString("agma", Fonts.getSecondary().drawSmoothString("M", 5.0, 5.0f, Color.white.darker().getRGB()) + 1.0f, 5.0f, Magma.themeManager.getSecondaryColor(0).getRGB());
                } else {
                    Fonts.getSecondary().drawSmoothString("(Unlicensed)", Fonts.getSecondary().drawSmoothString("ore", Fonts.getSecondary().drawSmoothString("K", 5.0, 5.0f, Color.white.darker().getRGB()) + 1.0f, 5.0f, Magma.themeManager.getSecondaryColor(0).getRGB()) + 3.0f, 5.0f,Color.white.darker().getRGB());
                }
            }
            if (this.arrayList.isEnabled()) {
                GL11.glPushMatrix();
                final ScaledResolution resolution = new ScaledResolution(Magma.mc);
                final List<Module> list = Magma.moduleManager.getModules().stream().filter(module -> (module.isToggled() || module.toggledTime.getTimePassed() <= 250L)).sorted(Comparator.comparingDouble(module -> Fonts.getPrimary().getStringWidth(module.getName() + " | " + module.getSuffix()))).collect(Collectors.toList());
                ...
                for (final Module module2 : list) {
                    ...
                    Fonts.getPrimary().drawSmoothCenteredString(moduleName, resolution.getScaledWidth() - 1 - width / 2.0f, y + height / 2.0f - Fonts.getPrimary().getHeight() / 2.0f, Magma.themeManager.getSecondaryColor(x).getRGB(), Magma.themeManager.getSecondaryColor(x - 1).getRGB());
                ...
                if (this.arrayOutline.isEnabled()) {
                    ...
                    for (final Module module3 : list) {
                        ...
                        addVertex((float)(resolution.getScaledWidth() - 1), y, (float)resolution.getScaledWidth(), y + height, Magma.themeManager.getSecondaryColor(x - 1).getRGB(), Magma.themeManager.getSecondaryColor(x).getRGB());
                        y += height;
                    }
                    tessellator.draw();
    ...
    public void onEnable()
    {
        ...
        Magma.mc.displayGuiScreen(modernClickGui);
    }

    @Override
    public void onDisable()
    {
        Magma.mc.displayGuiScreen(null);
    }

    public Color getColor() {
        return Magma.themeManager.getSecondaryColor(0);
    }

    public Color getColor(int index) {
        return Magma.themeManager.getSecondaryColor(index);
    }
}
