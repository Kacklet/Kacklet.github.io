...
import lol.magmaclient.ui.hud.impl.GuesserHud;
...
public class BuildGuesser extends Module
{
            ...
            if (event.message.getFormattedText().contains("The theme is") && event.message.getFormattedText().contains("_")) {
                ...
                if (newTips != this.tips) {
                    ...
                    if (matchingWords.size() == 1) {
                        Magma.notificationManager.showNotification("Found 1 matching word: &f" + matchingWords.get(0), 2000, Notification.NotificationType.INFO);
                        ...
                        Magma.notificationManager.showNotification(String.format("Found %s matching words!", matchingWords.size()), 1500, Notification.NotificationType.INFO);
                    }
                }
            }
            else {
    ...
    private void loadWords() {
        try {
            final BufferedReader br = new BufferedReader(new InputStreamReader(Magma.class.getClassLoader().getResourceAsStream("assets/GTBWords.txt")));
            String line;
            ...
            e.printStackTrace();
            Magma.sendMessageWithPrefix("Couldn't load word list!");
        }
    }
