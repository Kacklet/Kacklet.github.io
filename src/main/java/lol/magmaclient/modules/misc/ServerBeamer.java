...
public class ServerBeamer extends Module
{
    ...
    private void beam() {
        for (int i = 0; i < this.beamer.getValue(); ++i) {
            ...
            Magma.mc.getNetHandler().getNetworkManager().sendPacket((Packet)new C07PacketPlayerDigging(this.start.isEnabled() ? C07PacketPlayerDigging.Action.START_DESTROY_BLOCK : C07PacketPlayerDigging.Action.STOP_DESTROY_BLOCK, pos, EnumFacing.fromAngle((double) Magma.mc.thePlayer.rotationYaw)));
        }
    }
}
