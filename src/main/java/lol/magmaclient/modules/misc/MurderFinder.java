package lol.magmaclient.modules.misc;
...
public class MurderFinder extends Module
{
    ...
    public void assign()
    {
        Magma.murderFinder = this;
    }

    public boolean hasLine(final String line) {
        if (Magma.mc.thePlayer != null && Magma.mc.thePlayer.getWorldScoreboard() != null && Magma.mc.thePlayer.getWorldScoreboard().getObjectiveInDisplaySlot(1) != null) {
    ...
    public void onTick(final TickEvent.ClientTickEvent event) {
        if (!this.isToggled() || Magma.mc.thePlayer == null || Magma.mc.theWorld == null) {
            return;
        }

        try {
            if (Magma.mc.thePlayer.getWorldScoreboard() != null) {
                final ScoreObjective objective = Magma.mc.thePlayer.getWorldScoreboard().getObjectiveInDisplaySlot(1);
                if (objective != null && ChatFormatting.stripFormatting(objective.getDisplayName()).equals("MURDER MYSTERY") && hasLine("Innocents Left:")) {
                    this.inMurder = true;
                    for (final EntityPlayer player : Magma.mc.theWorld.playerEntities) {
                        if (!murderers.contains(player)) {
                            ...
                            if (detectives.size() < 2 && player.getHeldItem().getItem().equals(Items.bow)) {
                                ...
                                Magma.sendMessageWithPrefix(String.format(Magma.fancy + "b%s is detective!", player.getName()));
                                Magma.notificationManager.showNotification(String.format(Magma.fancy + "b%s is detective!", player.getName()), 5000, Notification.NotificationType.WARNING);
                            }
                            if (!this.knives.contains(player.getHeldItem().getItem())) {
                                continue;
                            }
                            murderers.add(player);
                            Magma.sendMessageWithPrefix(String.format(Magma.fancy + "c%s is murderer!", player.getName()));
                            Magma.notificationManager.showNotification(String.format(Magma.fancy + "c%s is murderer!", player.getName()), 10000, Notification.NotificationType.WARNING);
                            if (!this.autoSay.isEnabled() || player == Magma.mc.thePlayer) {
                                continue;
                            }
                            Magma.mc.thePlayer.sendChatMessage(String.format("%s is murderer!", ChatFormatting.stripFormatting(player.getName())));
                        }
                    }
                    return;
                }
    ...
    public void onWorldRender(final RenderWorldLastEvent e) {
        ...
        if (this.inMurder) {
            for (final Entity entity : Magma.mc.theWorld.loadedEntityList) {
                if (entity instanceof EntityPlayer) {
                    if (((EntityPlayer)entity).isPlayerSleeping()) {
                        continue;
                    }
                    if (entity == Magma.mc.thePlayer) {
                        continue;
                    }
