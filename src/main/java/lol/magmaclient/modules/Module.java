...
import lol.magmaclient.managers.ConfigManager;
...
public class Module {
    ...
    public void riskWarning() {
        if(this.flagType == FlagType.DETECTED) {
            Magma.notificationManager.showNotification("This module is detected, use it carefully", 2500, Notification.NotificationType.WARNING);
        }
    }
    ...
    public static List<Module> getModulesByCategory(final Category c) {
        return (List<Module>) Magma.moduleManager.getModules().stream().filter(module -> module.category == c).collect(Collectors.toList());
    }
