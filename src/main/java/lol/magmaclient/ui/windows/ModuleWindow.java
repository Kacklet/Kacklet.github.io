...
import lol.magmaclient.ui.components.*;
...
public class ModuleWindow extends Window {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (!ModernClickGui.settingsOpened) {
            for (Module module : this.modulesInCategory) {
                ...
                {
                    if(Magma.clientSettings.hideModules.getSelected().equals("Detected") || Magma.clientSettings.hideModules.getSelected().equals("Premium + Detected")) {
                        continue;
                    }
                }
                ...
                {
                    if(Magma.clientSettings.hideModules.getSelected().equals("Premium") || Magma.clientSettings.hideModules.getSelected().equals("Premium + Detected")) {
                        continue;
                    }
                }

                RenderUtils.drawBorderedRoundedRect((float)(ModernClickGui.getX() + 95.0), (float)(ModernClickGui.getY() + (double)offset + this.scrollAnimation.getValue()), ModernClickGui.getWidth() - 100.0f, 20.0f, 3.0f, 1.0f, module.isToggled() ? Magma.themeManager.getSecondaryColor().getRGB() : Magma.themeManager.getPrimaryColor().getRGB(), Magma.themeManager.getSecondaryColor().getRGB());
                    ...
                    switch (mouseButton) {
                        case 0: {
                            ...
                            if(!module.isToggled() && module.getVersionType() == Module.VersionType.PREMIUM) {
                                if(Magma.licenseManager != null && Magma.licenseManager.isPremium()) {
                                    module.toggle();
                                } else {
                                    Magma.sendMessageWithPrefix("You need to be premium to use that.");
                                }
                            } else {
                                module.toggle();
                            }
    ...
    public void keyTyped(char typedChar, int keyCode) {
        if (keyCode == Magma.clickGui.getKeycode())
        {
            if (changeBind != null)
            {
