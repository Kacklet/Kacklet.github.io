...
import lol.magmaclient.modules.render.InventoryDisplay;
...
public class InventoryHud extends DraggableComponent
{
    ...
    public InventoryHud() {
        ...
        this.setPosition(Magma.inventoryDisplay.x.getValue(), Magma.inventoryDisplay.y.getValue());
    }

    public HudVec drawScreen() {
        GL11.glPushMatrix();
        super.drawScreen();
        final ScaledResolution scaledResolution = new ScaledResolution(Magma.mc);
        int blur = 0;
        ...
        final String selected = Magma.inventoryDisplay.blurStrength.getSelected();
        ...
        for (int i = 9; i < 36; ++i) {
            ...
            if (stack != null) {
                Magma.mc.getRenderItem().renderItemAndEffectIntoGUI(stack, (int)x + 2 + 20 * (i % 9), (int)y);
        ...
        Magma.inventoryDisplay.x.set(this.x);
        Magma.inventoryDisplay.y.set(this.y);
