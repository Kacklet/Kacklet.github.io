package lol.magmaclient.utils.render;

import lol.magmaclient.Magma;
import lol.magmaclient.utils.WorldToScreen;
...
//        GL11.glHint(3155, 4352);
    ...
    public static void scissor(double x, double y, double width, double height) {
        final ScaledResolution sr = new ScaledResolution(Magma.mc);
    ...
    public static void draw2D(final Entity entityLiving, final float partialTicks, final float lineWidth, final Color color) {
        ...
        GL11.glPushMatrix();
        GL11.glLoadIdentity();
        GL11.glOrtho(0.0, (double) Magma.mc.displayWidth, (double) Magma.mc.displayHeight, 0.0, -1.0, 1.0);
        ...
        final RenderManager renderManager = Magma.mc.getRenderManager();
        ...
        for (final double[] boxVertex : boxVertices) {
            final Vector2f screenPos = WorldToScreen.worldToScreen(new Vector3f((float)boxVertex[0], (float)boxVertex[1], (float)boxVertex[2]), mvMatrix, projectionMatrix, Magma.mc.displayWidth, Magma.mc.displayHeight);
            if (screenPos != null) {
        ...
        if (minX > 0.0f || minY > 0.0f || maxX <= Magma.mc.displayWidth || maxY <= Magma.mc.displayWidth) {
            GL11.glColor4f(color.getRed() / 255.0f, color.getGreen() / 255.0f, color.getBlue() / 255.0f, 1.0f);
            GL11.glBegin(2);
    ...
    public static void drawTargetESP(final EntityLivingBase target, final Color color, final float partialTicks) {
        ...
        GlStateManager.translate(target.lastTickPosX + (target.posX - target.lastTickPosX) * partialTicks - Magma.mc.getRenderManager().viewerPosX, target.lastTickPosY + (target.posY - target.lastTickPosY) * partialTicks - Magma.mc.getRenderManager().viewerPosY + target.height * location, target.lastTickPosZ + (target.posZ - target.lastTickPosZ) * partialTicks - Magma.mc.getRenderManager().viewerPosZ);
        enableGL2D();
    ...
    public static void drawImage(final ResourceLocation image, final float x, final float y, final float width, final float height, final float alpha) {
        ...
        Magma.mc.getTextureManager().bindTexture(image);
    ...
    public static void renderEspBox(BlockPos blockPos, float partialTicks, int color, float opacity) {
        if (blockPos != null) {
            IBlockState blockState = Magma.mc.theWorld.getBlockState(blockPos);

            if (blockState != null) {
                Block block = blockState.getBlock();
                block.setBlockBoundsBasedOnState(Magma.mc.theWorld, blockPos);
                double d0 = Magma.mc.thePlayer.lastTickPosX + (Magma.mc.thePlayer.posX - Magma.mc.thePlayer.lastTickPosX) * (double) partialTicks;
                double d1 = Magma.mc.thePlayer.lastTickPosY + (Magma.mc.thePlayer.posY - Magma.mc.thePlayer.lastTickPosY) * (double) partialTicks;
                double d2 = Magma.mc.thePlayer.lastTickPosZ + (Magma.mc.thePlayer.posZ - Magma.mc.thePlayer.lastTickPosZ) * (double) partialTicks;
                drawFilledBoundingBox(block.getSelectedBoundingBox(Magma.mc.theWorld, blockPos).expand(0.002D, 0.002D, 0.002D).offset(-d0, -d1, -d2), color, opacity);
            }
        }
    }
    ...
    public static void blockBox(final TileEntity block, final Color color) {
        ...
        GL11.glDepthMask(false);
        setColor(color);
        RenderGlobal.drawSelectionBoundingBox(block.getRenderBoundingBox().offset(-Magma.mc.getRenderManager().viewerPosX, -Magma.mc.getRenderManager().viewerPosY, -Magma.mc.getRenderManager().viewerPosZ));
        GL11.glEnable(3553);
        GL11.glEnable(2929);
        GL11.glDepthMask(true);
        GL11.glDisable(3042);
    ...
    public static void renderStarredNametag(final Entity entityIn, final String str, final double x, final double y, final double z, final int maxDistance) {
        final double d0 = entityIn.getDistanceSqToEntity(Magma.mc.getRenderManager().livingPlayer);
        if (d0 <= maxDistance * maxDistance) {
            final FontRenderer fontrenderer = Magma.mc.getRenderManager().getFontRenderer();
            final float f = 1.6f;
    ...
    public static void renderLivingLabel(final Entity entityIn, final String str, final double x, final double y, final double z, final int maxDistance) {
        final double d0 = entityIn.getDistanceSqToEntity(Magma.mc.thePlayer);
        if (d0 <= maxDistance * maxDistance) {
            final FontRenderer fontrenderer = Magma.mc.getRenderManager().getFontRenderer();
            final float f = 1.6f;
            final float f2 = 0.016666668f * f;
