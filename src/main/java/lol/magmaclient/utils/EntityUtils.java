...
public class EntityUtils
{
    private static boolean isOnTeam(final EntityPlayer player) {
        for (final Score score : Magma.mc.thePlayer.getWorldScoreboard().getScores()) {
    ...
    public static boolean isTeam(final EntityLivingBase e2) {
        ...
        return Magma.mc.thePlayer.getDisplayName().getFormattedText().charAt(2) == Magma.fancy && e2.getDisplayName().getFormattedText().charAt(2) == '?' && Magma.mc.thePlayer.getDisplayName().getFormattedText().charAt(3) == e2.getDisplayName().getFormattedText().charAt(3);
    }
