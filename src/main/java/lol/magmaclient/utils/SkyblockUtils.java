...
public class SkyblockUtils {
    ...
    public void onTick(final TickEvent.ClientTickEvent event) {
        if (Magma.mc.theWorld != null && event.phase == TickEvent.Phase.START) {
    ...
    {
        return Magma.mc.getCurrentServerData().serverIP.endsWith("hypixel.net");
    }

    public static boolean isOnSkyBlock() {
        try {
            final ScoreObjective titleObjective = Magma.mc.thePlayer.getWorldScoreboard().getObjectiveInDisplaySlot(1);
            if (Magma.mc.thePlayer.getWorldScoreboard().getObjectiveInDisplaySlot(0) != null) {
                return ChatFormatting.stripFormatting(Magma.mc.thePlayer.getWorldScoreboard().getObjectiveInDisplaySlot(0).getDisplayName()).contains("SKYBLOCK");
            }
            return ChatFormatting.stripFormatting(Magma.mc.thePlayer.getWorldScoreboard().getObjectiveInDisplaySlot(1).getDisplayName()).contains("SKYBLOCK");
        }
        catch (Exception e) {
            return false;
        }
    ...
    public static boolean isInOtherGame() {
        try {
            final Scoreboard sb = Magma.mc.thePlayer.getWorldScoreboard();
            final List<Score> list = new ArrayList<Score>(sb.getSortedScores(sb.getObjectiveInDisplaySlot(1)));
            for (final Score score : list) {
                final ScorePlayerTeam team = sb.getPlayersTeam(score.getPlayerName());
    ...
    {
        if (Magma.mc.thePlayer == null) return new ArrayList<>();
        return playerInfoOrdering.sortedCopy(Magma.mc.thePlayer.sendQueue.getPlayerInfoMap());
    }
